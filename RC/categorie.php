...
<?php
 if(isset($_POST['add_cat'])){
   ...
   if(empty($errors)){
      $sql  = "INSERT INTO ubicaciones (nombre)";
      $sql .= " VALUES ('{$cat_name}')";
      if($db->query($sql)){
      ...
      <div class="panel panel-default">
        <div class="panel-heading">
          <strong>
            ...
            <span>Agregar estantes</span>
         </strong>
        </div>
        <div class="panel-body">
          <form method="post" action="categorie.php">
            <div class="form-group">
                <input type="text" class="form-control" name="categorie-name" placeholder="Nombre del estante" required>
            </div>
            <button type="submit" name="add_cat" class="btn btn-primary">Agregar estante</button>
        </form>
        </div>
      </div>
    </div>
