  ...
  if(isset($_POST['update'])){
    ...
    if(empty($errors)){
       ...
       $username = remove_junk($db->escape($_POST['username']));
            $sql = "UPDATE usuarios SET nombre ='{$name}', username ='{$username}' WHERE id='{$id}'";
    $result = $db->query($sql);
      ...
      <div class="panel-body">
          <form method="post" action="edit_account.php?id=<?php echo (int)$user['id'];?>" class="clearfix">
            <div class="form-group">
                  ...
                  <input type="name" class="form-control" name="name" value="<?php echo remove_junk(ucwords($user['nombre'])); ?>">
            </div>
            <div class="form-group">
