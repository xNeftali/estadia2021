...
<?php
  ...
  $groups  = find_all('usuarios_grupos');
  if(!$e_user){
  ...
  if(isset($_POST['update'])) {
    ...
    if(empty($errors)){
       ...
       $status   = remove_junk($db->escape($_POST['status']));
            $sql = "UPDATE usuarios SET nombre ='{$name}', username ='{$username}',usuario_nivel='{$level}',status='{$status}' WHERE id='{$db->escape($id)}'";
         $result = $db->query($sql);
          if($result && $db->affected_rows() === 1){
            $session->msg('s',"Cuenta actualizada ");
            redirect('edit_user.php?id='.(int)$e_user['id'], false);
          } else {
            $session->msg('d',' Lo siento no se actualizó la cuenta.');
            redirect('edit_user.php?id='.(int)$e_user['id'], false);
          }
    } else {
       ...
       <div class="panel-body">
          <form method="post" action="edit_user.php?id=<?php echo (int)$e_user['id'];?>" class="clearfix">
            <div class="form-group">
                  ...
                  <input type="name" class="form-control" name="name" value="<?php echo remove_junk(ucwords($e_user['nombre'])); ?>">
            </div>
            <div class="form-group">
              ...
              <label for="level">Rol de usuario</label>
                <select class="form-control" name="level">
                  <?php foreach ($groups as $group ):?>
                   <option <?php if($group['grupo_nivel'] === $e_user['usuario_nivel']) echo 'selected="selected"';?> value="<?php echo $group['grupo_nivel'];?>"><?php echo ucwords($group['grupo_nombre']);?></option>
                <?php endforeach;?>
                </select>
            </div>
            <div class="form-group">
